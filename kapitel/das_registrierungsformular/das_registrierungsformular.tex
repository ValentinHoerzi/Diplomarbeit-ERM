...
Diese Methode für die Standorterkennung wird durch einen Klick auf "Use my location" ausgelöst. Bevor jedoch der Standort abgerufen wird, wird der Benutzer nach der Erlaubnis gefragt.\autocite{useMyLocation}\\
Die Standortermittlung funktioniert mit der Methode \texttt{getCurrentPosition()} des \texttt{navigator.geolocation}-Objektes, die man im Listing \ref{lst:useMyLocation} sieht. Dabei wird eine asynchrone Abfrage gestartet, die die Position abruft. Bei erfolgreichen Abschluss dieser Abfrage, wird die mitgegebene Funktion mit der gefundene Position ausgeführt. Als optionalen zweiten Parameter kann dabei auch eine Funktion mitgegeben werden, die nur bei eintritt eines Fehlers aufgerufen wird. \autocite{useMyLocation}
